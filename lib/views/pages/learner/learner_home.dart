import 'package:flutter/material.dart';

import 'package:mentra_mobile_view/components/Learner/home_header.dart';
import 'package:mentra_mobile_view/components/Learner/learner_bottomnav.dart';
import 'package:mentra_mobile_view/components/Learner/learner_topnavbar.dart';
import 'package:mentra_mobile_view/components/Learner/microtraining_filters.dart';
import 'package:mentra_mobile_view/models/learner/microtraining_model.dart';
import 'package:mentra_mobile_view/services/API/learner/api_service.dart';
import 'package:mentra_mobile_view/services/API/learner/microtraining_service.dart';
import 'package:mentra_mobile_view/services/storage_service.dart';
import 'package:mentra_mobile_view/views/pages/learner/feed_view.dart';
import 'package:mentra_mobile_view/views/pages/learner/microtraining_view.dart';

class LearnerHome extends StatefulWidget {
  const LearnerHome({super.key});

  @override
  State<LearnerHome> createState() => _LearnerHomeState();
}

class _LearnerHomeState extends State<LearnerHome> {
  int _currentNavIndex = 0;
  bool _isFeedSelected = true;
  String _selectedStatus = 'pending';
  final TextEditingController _searchController = TextEditingController();

  late Future<List<dynamic>> _feedFuture;
  late Future<List<MicrotrainingModel>> _microtrainingFuture;

  @override
  void initState() {
    super.initState();
    _loadFeed();
    _loadMicrotrainings();
  }

  @override
  void dispose() {
    _searchController.dispose();
    super.dispose();
  }

  void _loadFeed() async {
    final token = await StorageService.getAccessToken();
    final sessionKey = await StorageService.getSessionKey();
    setState(() {
      _feedFuture = ApiService.fetchFeedPosts(
        accessToken: token ?? '',
        sessionKey: sessionKey ?? '',
      );
    });
  }

  void _loadMicrotrainings() async {
    final token = await StorageService.getAccessToken();
    setState(() {
      _microtrainingFuture = MicrotrainingService.getMicrotrainings(
        accessToken: token ?? '',
        status: _selectedStatus,
        searchQuery: _searchController.text,
      ).then((dataList) => dataList.map((json) => MicrotrainingModel.fromJson(json)).toList());
    });
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8FAFC),
      appBar: const LearnerTopNavbar(),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Header Component
            HomeHeader(
              isFeedSelected: _isFeedSelected,
              onSelectMicrotrainings: () => setState(() => _isFeedSelected = false),
              onSelectFeed: () => setState(() => _isFeedSelected = true),
            ),
            const SizedBox(height: 20),

            // Filters Component (Microtrainings only)
            if (!_isFeedSelected) ...[
              MicrotrainingFilters(
                selectedStatus: _selectedStatus,
                searchController: _searchController,
                onStatusChanged: (newStatus) {
                  setState(() {
                    _selectedStatus = newStatus;
                    _loadMicrotrainings();
                  });
                },
                onSearchChanged: (_) => _loadMicrotrainings(),
              ),
              const SizedBox(height: 16),
            ],

            // Content Views
            _isFeedSelected
                ? FeedView(feedFuture: _feedFuture)
                : MicrotrainingView(microtrainingFuture: _microtrainingFuture),
          ],
        ),
      ),
      bottomNavigationBar: LearnerBottomNav(
        currentIndex: _currentNavIndex,
        onTap: (index) => setState(() => _currentNavIndex = index),
      ),
    );
  }
}